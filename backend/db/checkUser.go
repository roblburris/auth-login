...
// func CheckUser(ctx context.Context, pool *pgxpool.Pool, email string, aud string, pwd string) bool{
...
// func getPwd(ctx context.Context, pool *pgxpool.Pool, aud string) (byte[], byte[])  {
//     conn, err := pool.Acquire(ctx)
//     if err != nil {
//         log.Printf("ERROR: unable to acquire connection from pool. %v\n", err)
//         return false
...
//     rows, err := tx.Query(ctx, GET_PW_SALT, "abc")
//     // err handling
//     if err != nil {
//         log.Printf("ERROR: Unable to get blog posts from DB. %v\n", err)
//         return false
